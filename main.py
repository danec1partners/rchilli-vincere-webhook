...
def home():
    return "✅ RChilli to Vincere webhook is alive (Fixed for array fields)"
...
def webhook():
    try:
        ...
        phone_list = parsed_data.get("PhoneNumber", [])

        email = email_list[0].get("EmailAddress") if email_list else "unknown@example.com"
        phone = phone_list[0].get("Number") if phone_list else "N/A"
